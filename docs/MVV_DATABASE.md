...
`data/mvv-ui-data.json` 是前端静态页面和其他 AI 可读取的机器规范导出。

`data/mvv-ui-data.js` 是同一份数据的浏览器包装，格式为：

```js
window.__MVV_DATA = { ... };
```

`index.html` 通过 `<script src="data/mvv-ui-data.js"></script>` 加载它。这个设计是刻意选择：它既保持数据与模板分离，又允许直接双击 `index.html` 在 `file://` 下查看页面。不要改回 `fetch("data/mvv-ui-data.json")`，否则本地双击会被浏览器安全策略拦截。

`data/mvv.sqlite` 是派生文件，由同步脚本本地重建，不提交到 git。主数据仍以 `data/companies/**`、`data/mvv-ui-data.json` 和 `data/mvv-ui-data.js` 为准。
